...
(function () {
  ...
  _p[0] = {
    value: function (require, exports, module) {
      ...
      connect.register(
        "arc",
        function (node, parent, connection, width, color) {
    ...
    value: function (require, exports, module) {
      ...
      function getSVGInfo(minder) {
        ...
        var paper = minder.getPaper(),
          ...
          width = (renderBox.width + 1) * ratio,
          height = (renderBox.height + 1) * ratio,
          blob,
          svgUrl,
          img;
        // 保存原始变换，并且移动到合适的位置
        paperTransform = paper.shapeNode.getAttribute("transform");
        paper.shapeNode.setAttribute(
          "transform",
          `translate(0.5, 0.5) scale(${ratio})`
        );
        ...
        // 回复原始变换及位置
        renderContainer.translate(renderBox.x, renderBox.y);
        paper.shapeNode.setAttribute("transform", `scale(1) ${paperTransform}`);
        // 过滤内容
        ...
        for (var i = 0; i < allNodes.length; i++) {
          ...
          if (nodeData.image) {
            ...
            var imageRenderBox = allNodes[i].getRenderBox(
              "ImageRenderer",
            ...
            var imageInfo = {
              ...
              x:
                (-renderContainer.getBoundaryBox().x + imageRenderBox.x) *
                  ratio +
                20,
              y:
                (-renderContainer.getBoundaryBox().y + imageRenderBox.y) *
                  ratio +
                20,
            };
      ...
      function encode(json, minder) {
        ...
        var width = svgInfo.width * ratio;
        var height = svgInfo.height * ratio;
